...
describe('Iterator', () => {
  ...
  test('position', () => {
    const iter = Iterator.from(['a', 'b', 'c', 'd']);
  ...
  test('zip', () => {
    ...
    const iter2 = Iterator.from(['a', 'b', 'c']);
    const zipped = iter1.zip(iter2);
    expect(zipped.collect().unwrap()).toEqual([[1, 'a'], [2, 'b'], [3, 'c']]);
  });

  test('unzip', () => {
    const iter = Iterator.from<[number, string]>([[1, 'a'], [2, 'b'], [3, 'c']]);
  ...
  test('sortBy', () => {
    const iter = Iterator.from([3, 1, 4, 1, 5, 9, 2, 6]);
